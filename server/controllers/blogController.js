        ...
        try {
            ...
            if (parsed?.error) {
                ...
                if (
                    ...
                    topMsg?.toLowerCase().includes("api key not valid")
                ) {
                    clean =
...
export const addBlog = async (req, res) => {
    ...
    try {
        ...
        await Blog.create({
            ...
            seoTips: Array.isArray(seoTips) ? seoTips : []
        });
...
export const analyzeContent = async (req, res) => {
    ...
    try {
        ...
        // Frontend may send either `content` or `description`
        const blogContent =
            content?.trim() ||
            description?.trim() ||
            "";
...
`;
        ...
        for (const field of scoreFields) {
            ...
            const score = Number(analysis[field]);
            ...
            analysis[field] = Math.round(score);
        }
...
User Suggestions:
${suggestions || "Make it clearer, more professional, engaging and easy to read."}
...
const aiStudioContent = (description) => String(description || "").trim();

export const generateSEO = async (req, res) => {
    try {
        const { title, subTitle, description, category } = req.body;
        if (!title?.trim() || !aiStudioContent(description)) {
            return res.json({ success: false, message: "Title and blog content are required" });
        }
...
`;
        ...
        const result = cleanAIJson(await main(prompt));
        res.json({ success: true, seo: result });
    } catch (error) {
        console.error("Generate SEO Error:", error);
        res.json({ success: false, message: extractErrorMessage(error) });
    }
};

export const thumbnailIdea = async (req, res) => {
    try {
...
`;
        ...
        const idea = cleanAIJson(await main(prompt));
        res.json({ success: true, idea });
    } catch (error) {
        console.error("Thumbnail Idea Error:", error);
        res.json({ success: false, message: extractErrorMessage(error) });
    }
};
...
export const quickAction = async (req, res) => {
    try {
        ...
        if (!action || !aiStudioContent(description)) {
            return res.json({ success: false, message: "Action and blog content are required" });
        }

        const instructions = {
            summarize: "Create a concise summary of the blog in 5-7 clear sentences.",
            intro: "Write a stronger, engaging introduction for this blog while keeping its topic and meaning.",
            social: "Create 3 short social media posts promoting this blog. Make them platform-friendly and engaging.",
            faq: "Create 5 useful FAQs with short answers based only on the blog content.",
            conclusion: "Write a strong conclusion that summarizes the key message and gives the reader a useful final takeaway."
        };

        const instruction = instructions[action];
        if (!instruction) {
            return res.json({ success: false, message: "Unsupported AI action" });
        }
...
Task: ${instruction}
Return only the requested content. Keep language simple, useful and professional.
`;
...
export const counterPerspective = async (req, res) => {
    try {
        const { title, description, category } = req.body;
        if (!aiStudioContent(description)) {
            return res.json({ success: false, message: "Blog content is required" });
        }
...
Return exactly:
{
  "assumptions": ["up to 3 assumptions"],
  "missingViewpoints": ["up to 3 missing viewpoints"],
  "counterArgument": "one balanced counter-argument"
}
Do not invent facts that are not reasonably implied by the blog.
`;

        const perspective = cleanAIJson(await main(prompt));
        res.json({ success: true, perspective });
    } catch (error) {
        console.error("Counter Perspective Error:", error);
        res.json({ success: false, message: extractErrorMessage(error) });
    }
};

export const askAboutBlog = async (req, res) => {
    try {
        const { question, title, description, category } = req.body;
        if (!question?.trim()) return res.json({ success: false, message: "Question is required" });
        if (!aiStudioContent(description)) return res.json({ success: false, message: "Blog content is required" });
...
export const changeTone = async (req, res) => {
    try {
        const { title, description, category, tone } = req.body;
        const allowed = ["professional", "friendly", "casual", "academic", "storytelling"];
        if (!allowed.includes(tone)) return res.json({ success: false, message: "Invalid tone" });
        if (!aiStudioContent(description)) return res.json({ success: false, message: "Blog content is required" });
...
export const generateIdeas = async (req, res) => {
    try {
        ...
        if (!topic?.trim() && !category?.trim()) {
            return res.json({ success: false, message: "Topic or category is required" });
        }
...
`;
        ...
        const result = cleanAIJson(await main(prompt));
        res.json({ success: true, ideas: Array.isArray(result.ideas) ? result.ideas : [] });
    } catch (error) {
...
export const updateBlog = async (req, res) => {
    ...
    try {
        const blogData = JSON.parse(req.body.blog || "{}");
        const { id, title, subTitle, description, category, isPublished } = blogData;
        const imageFile = req.file;

        if (!id || !title?.trim() || !description?.trim() || !category?.trim()) {
            return res.json({ success: false, message: "Blog id, title, content and category are required" });
        }
        ...
        const update = {
            ...
            metaTitle: blogData.metaTitle || blog.metaTitle,
            metaDescription: blogData.metaDescription || blog.metaDescription,
            slug: blogData.slug || blog.slug,
            focusKeyword: blogData.focusKeyword || blog.focusKeyword,
            seoKeywords: Array.isArray(blogData.seoKeywords) ? blogData.seoKeywords : blog.seoKeywords,
            seoTips: Array.isArray(blogData.seoTips) ? blogData.seoTips : blog.seoTips,
        };
        ...
        if (imageFilePath) {
            try { fs.unlinkSync(imageFilePath); } catch (_) {}
        }
    }
};
// =====================================================
